...
- **Templater**: permite crear plantillas dinámicas (fechas automáticas, título del archivo, cursores inteligentes). Es la base para que las plantillas de apuntes funcionen.
...
- **Advanced Tables (table-editor-obsidian)**: facilita crear y editar tablas en Markdown con atajos de teclado (Tab para moverte entre celdas, autoformateo). Útil para las tablas de comandos/funciones de cada lenguaje. *(Nota: quedó deshabilitado en tu vault — actívalo en Configuración → Community plugins si lo quieres usar).*

Al abrir el vault en Obsidian, los plugins ya deberían aparecer habilitados en **Configuración → Community plugins**. Si Obsidian pide confirmación de seguridad la primera vez, es normal (son plugins de código abierto descargados directo de sus repositorios oficiales en GitHub).
...
Para listar todos tus apuntes de lenguajes en una tabla, puedes crear una nota y pegar esto (con el bloque marcado como `dataview`, no genérico):
